...
public class Player
{
    ...
    public class Dashboard
    {
        ...
        public List<GhostMemory> GhostMemories = new List<GhostMemory>();
        //
    ...
    void updateStun()
    {
        ...
        //
        //Dashboards[Round].StunnedBusterIDs;
        //Dashboards[Round].StunnedEnemyBusterIDs;
    ...
    #region Memory
    public static Point ProjectGhost(int ghostX, int ghostY, int busterX, int busterY)
    {
        try
        {
            int dx = ghostX - busterX; int dy = ghostY - busterY;
            int distance = Distance(ghostX, ghostY, busterX, busterY);
            int x = ghostX + Convert.ToInt32(Math.Round(1.0 * dx * LimGhostSpeed / distance));
            int y = ghostY + Convert.ToInt32(Math.Round(1.0 * dy * LimGhostSpeed / distance));
            return new Point(Math.Max(0, Math.Min(W - 1, x)), Math.Max(0, Math.Min(H - 1, y)));
        }
        catch { return new Point(ghostX, ghostY); }
    }
    public struct GhostMemory
    {
        public int GhostID, Round, X, Y, Stamina;
    }
    #endregion
    ...
    public List<string> GetMoves()
    {
        ...
        if (Round > 32) limBustStamina = 100;
        ...
        foreach (int ghostID in dashboard.ContestedGhostsIDs)
        {
            ...
            //
            if (numEnemyBusters == busters) continue;
            //
        ...
        {
            //
            if (dashboard.DispatchActions.Where(d => d.Value.EntityID == e.ID).Count() > 0) continue;
            //
            ...
            //
            roundGhostMemories.Add(new GhostMemory() { GhostID = e.ID, Round = Round, X = p.X, Y = p.Y, Stamina = esGhost.State });
        }
        if (Round > 0)
        {
        ...
        foreach (GhostMemory gm in previousGhostMemories)
        {
            ...
            if (roundGhostIDs.Contains(gm.GhostID)) continue;
            ...
            if (!found) roundGhostMemories.Add(gm);
        }
        foreach (GhostMemory gm in roundGhostMemories.Where(g => g.Stamina < limBustStamina).OrderBy(g => g.Stamina))
        {
            foreach (string busterID in BusterIDs.Where(bid => !dashboard.DispatchActions.ContainsKey(bid)))
            {
                DispatchAction action = new DispatchAction() { Type = DispatchActionType.Find,
